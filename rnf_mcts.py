...
class Node():
    ...
    def expand(self, network):
        ...
        q = network.predict(np.array([toNetworkInput(self.board)]))
        prob_sum = 0.
        for (edge,_) in self.childEdgeNode:
            m_idx = getNetworkOutputIndex(edge.move)
            edge.P = q[0][0][m_idx]
            prob_sum += edge.P
        for edge,_ in self.childEdgeNode:
            edge.P /= prob_sum
...
class MCTS():
    ...
    def select(self, node):
        if(node.isLeaf()):
            ...
            for edge, child_node in node.childEdgeNode:
                ...
                val = edge.Q
                if(edge.parentNode.board.player_to_move == O):
                    val = -edge.Q
    ...
    def expandAndEvaluate(self, node):
        terminal, winner = node.board.game_over, node.board.game_result(node.global_cells)
        if(terminal == True):
            v = 0.0
            if(winner == X):
                v = 1.0
